...
- 상세형(풍부하게)으로 작성: ✅ 항목 6~8개, 각 항목 대시 문장 4~6줄, 마지막 핵심정리 4~5개.
- 내용은 풍부하게 담되 문장 하나하나는 짧고 압축적으로 — 항목과 대시 줄 수로 분량을 늘린다.
...
- 구성: ✳ 2줄 헤드라인(가장 강한 입지 포인트 + 업종 연결 매력 포인트)
  → ✅ 핵심 입지 / ✅ 배후수요 / ✅ 유동 동선(출근·등교·점심·하교·귀가 등 시간대별) / ✅ 업종과의 궁합
  / ✅ 경쟁력(정보 있을 때) / ✅ 이 매물의 포인트(핵심 강점 4~6개 압축) / ✅ 핵심 정리(4~5개 번호)
  → 마지막은 반드시 한 줄 요약으로 마무리.`

// ── 9번 프랜차이즈 작성 기준 (브랜드 자체만 — 입지·상권·주소 금지) ──
const FRANCHISE_GUIDE = `
[프랜차이즈 브랜드 분석 작성 기준]
- 프랜차이즈 브랜드 자체의 경쟁력만 정리한다. 입지·상권·주소·매물 정보는 절대 쓰지 않는다.
- 풍부하게 작성: ✅ 항목 5~6개, 각 항목 대시 문장 3~5줄. 문장은 짧게, 항목 수로 내용을 채운다.
- 구성: ✅ 브랜드 경쟁력(인지도·가맹망 규모·본사 노하우·트렌드 적합성·경쟁 브랜드 대비 차별점)
  / ✅ 메뉴·상품 경쟁력(대표 메뉴·객단가 상승 포인트·세트/사이드 구조·반복 구매·계절/시간대 수요 대응)
  / ✅ 본사 운영 시스템(교육·조리/운영 매뉴얼·슈퍼바이저·물류/원재료·마케팅/앱/프로모션·신메뉴 개발)
  / ✅ 매출 구조 장점(홀·포장·배달 여부·회전율·시간대별 매출·재방문 수요·고객층 확장성)
  / ✅ 인수창업 메리트(브랜드 인지도 활용·시스템 승계·기존 고객/리뷰/배달앱 데이터·시설 완비·초보 접근성)
  / ✅ 핵심 정리(3~5개 번호) + 마지막 한 줄 요약.
...
✅ 시설 상태 — 제공된 시설 상태 내용을 반영, 인수자의 추가 투자 부담 관점. 신규급이면 "별도 시설투자 없이 바로 운영에 집중하기 좋은 상태", 오래됐으면 "선택적 보완 중심으로 접근 가능".
...
- 분량: 내용은 풍부하게 — 항목은 많게, 문장은 짧게. 각 ✅ 섹션은 대시 문장 4~6줄로 충실히 채우고,
  "이 매물의 포인트"는 5~6개, "핵심 정리"는 4~5개 번호로 작성한다.
  제공된 숫자·조건·특장점은 하나도 빠뜨리지 말고, 업종의 일반적인 강점(사실 수준)으로 살을 붙여도 좋다.
  단, 제공되지 않은 구체적 수치·시설·계약 조건을 지어내는 것은 금지.
...
${FRANCHISE_GUIDE}`
      ...
      const numbered = [
        ...
        ['10. 시설 상태', f.facility],
        ['11. 매출과 수익', f.revenue], ['12. 특장점', f.strengths], ['13. 특이사항 및 매도사유', f.notes],
