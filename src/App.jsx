  ...
  return (
    <ContractProvider>
      ...
      {(screen === 'step1' || screen === 'list') && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white border-t z-50 flex safe-area-bottom">
          <button
            type="button"
            onClick={() => setScreen('step1')}
            className={`flex-1 py-4 text-base font-semibold transition ${
              screen === 'step1' ? 'text-blue-600' : 'text-gray-400'
            }`}
          >
            계약서 작성
          </button>
          <button
            ...
            className={`flex-1 py-4 text-base font-semibold transition ${
              screen === 'list' ? 'text-blue-600' : 'text-gray-400'
            }`}
          >
            목록
          </button>
