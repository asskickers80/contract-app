...
// 특약사항 — 페이지 최상단 여백(상단 테두리와 "(A청색NCR…)" 인쇄줄 사이 빈 띠)
// 2026-08-25 대표님 지시로 추가. 입력했을 때만 그린다 (비우면 원본 그대로 출력).
// 기존 확정 좌표(POS/RECTS)는 일절 건드리지 않음.
export const SPECIAL_TERMS = {
  x0: 0.085,      // 왼쪽 시작 (표 왼쪽 여백과 동일)
  x1: 0.915,      // 오른쪽 한계
  yFirst: 0.0235, // 첫 줄 baseline
  lineH: 0.0140,  // 줄 간격 (페이지 높이 비율)
  size: 8,        // 기본 글자 크기(pt) — 안 들어가면 6.5pt까지 자동 축소
  maxLines: 2,    // 빈 띠에 안전하게 들어가는 최대 줄 수
}

// 이미지 영역(비율 사각형): x,y = 좌상단, w,h = 크기
// ⚠️ 2026-07-12 대표님 검수로 확정 — 대표님 지시 없이 수정 금지.
// 직인 오버레이는 없음 — 원본 스캔에 이미 회사 직인이 찍혀 있다.
export const RECTS = {
  handwriting: { x: 0.75625, y: 0.845, w: 0.150, h: 0.050 }, // 자필 확인 란 ('들었음' 워터마크 위)
  signature:   { x: 0.408, y: 0.930, w: 0.085, h: 0.033 }, // 성명 옆 빈칸
}
